...
export default function QRTypes() {
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      if (newImage) {
        ...
        const uploadRes = await axios.post(`${API_BASE}/admin/qr-types/upload-image`, formData, authHeader);
      ...
      const res = await axios.post(
        `${API_BASE}/admin/qr-types`,
        {
          ...
          copiesPerSet: 1,
          templateImage: uploadedImageUrl
        },
        authHeader
      );
      if (res.data.success) {
  ...
  const handleUpdate = async (e) => {
    ...
    setUpdating(true);
    try {
      let uploadedImageUrl = editingType.templateImage;
      if (editImage) {
        const formData = new FormData();
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  >
                    <span className="text-xs">🧳 Non-Vehicle (Tag ID)</span>
                    <span className="text-[10px] text-slate-500 mt-1 font-normal">Uses the last 4 digits of the Tag ID for verification</span>
                  </button>
                </div>
              </div>
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">
                  Template Image (Background for QR)
                </label>
                <div className="flex items-center space-x-3">
                  ...
                  <input
                    ...
                    className="block w-full text-xs text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-blue-50 file:text-[#1D56A5] hover:file:bg-blue-100 cursor-pointer"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="flex items-center space-x-3">
                  <div className="w-12 h-12 rounded-xl bg-slate-100 border border-slate-200 flex items-center justify-center overflow-hidden flex-shrink-0">
                    ...
                    ) : editingType?.templateImage ? (
                      <img src={editingType.templateImage} alt="Current" className="w-full h-full object-cover" />
                    ) : (
                      <ImageIcon className="w-5 h-5 text-slate-400" />
                    )}
                  </div>
