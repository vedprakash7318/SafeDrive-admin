    ...
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-black text-slate-900 flex items-center space-x-2.5">
            <Layers2 className="w-7 h-7 text-indigo-600" />
            <span>QR Product Types Management</span>
          </h1>
          <p className="text-xs text-slate-500 mt-1">
            Configure QR product types and set how many copies are generated per set (e.g. Car = 2, Bike = 1, Luggage = 10)
          </p>
        </div>
      ...
      <div className="flex items-center space-x-2 border-b border-slate-200 pb-2">
        ...
        >
          ...
          <span>🗑️ Soft-Deleted / Trash</span>
        </button>
      </div>
      ...
      <div className="bg-white border border-slate-200 rounded-3xl overflow-hidden shadow-xs">
        <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
          <span className="text-xs font-bold text-slate-700 uppercase tracking-wider">
            {viewTab === 'active' ? `Active QR Types (${types.length})` : `Soft-Deleted QR Types (${types.length})`}
          </span>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            <thead className="bg-slate-50 text-slate-500 uppercase text-[10px] font-black tracking-wider border-b border-slate-100">
              <tr>
                <th className="px-6 py-4">QR Type Name</th>
                <th className="px-6 py-4">Copies Generated Per Set</th>
                <th className="px-6 py-4">Total Sets Created</th>
              ...
              <div>
                ...
                <div className="flex items-center space-x-3">
                  <input
                    ...
                    className="w-28 bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-slate-900 text-sm font-bold focus:bg-white focus:outline-none focus:border-indigo-600 transition"
                    required
                  />
      ...
      {deleteTarget && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white border border-slate-200 rounded-3xl max-w-sm w-full p-6 shadow-2xl text-center">
            ...
            <h3 className="font-black text-xl text-slate-900 mb-2">Delete QR Type?</h3>
